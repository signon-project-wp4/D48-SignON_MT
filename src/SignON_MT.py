...
class SignON_MT:
    def __init__(self, model_type : int):
        ...
        ----------
        model_type : int
            Integer to select model according to MODEL_TYPE_NAME_MAPPER
        ...
        """
        assert isinstance(model_type, int), 'model_type should be a int [0,14]'
        assert model_type in MODEL_TYPE_NAME_MAPPER.keys(), 'Model type not recognized'
        self.tokenizer = spm.SentencePieceProcessor(model_file='./models/spm_mbart.model')
            
        # LOADING APPROACH 1 MAPPERS AND MODEL WEIGHTS
        self.model_name = MODEL_TYPE_NAME_MAPPER[model_type]
    ...
    def load_mbart(self, vocab_size: int = None, in_cpu: bool  = True) -> (TFSharedEmbeddings, TFMBartEncoder, TFMBartDecoder):
        ...
        with open(os.path.join('./utils/mbart_model/','config_architecture.dict'), 'rb') as f:
